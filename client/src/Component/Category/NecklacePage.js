...
const NecklacePage = () => {
    ...
    useEffect(() => {
        ...
        axios.get("/api/product/categoryList", {
            params: { 
                categoryId: 2,
                subCategory: subCategoryFromURL !== "전체" ? subCategoryFromURL : null
            }
        })
        .then((result) => {
    ...
    const handleSubCategoryClick = (subCategory) => {
        ...
        navigate(`/necklace?subCategory=${encodeURIComponent(subCategory || "전체")}`);
    };
        ...
        <article>
            <div className="sub-category-container">
                <div className="sub-category-buttons">
                    {["전체", "일체형", "메달형", "펜던트", "골드", "실버"].map((subCategory) => (
                        <button
                            key={subCategory}
