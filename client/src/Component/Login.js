...
const Login = () => {
    ...
    const KAKAO_AUTH_URL = "https://kauth.kakao.com/oauth/authorize?client_id=6ee1731553a983102257108c54fe99bc&redirect_uri=http://localhost:3000/login&response_type=code";
    ...
    useEffect(() => {
        ...
        const kakao_login= async()=>{
            let result= await axios.get(`/api/member/kakaoLogin?code=${kakaoCode}`, { withCredentials: true })
            if(result.data){
                ...
                result=await axios.post('/api/member/loginLocal',null,{params:{username: kakaoLoginUser.memberId, password: 'kakao'}})
                if(result.data){
    ...
    const onloginlocal = (e) => {
        ...
        axios.post("/api/member/loginLocal", null, { 
        ...
        .then(response => {
            ...
            if (typeof loginUser === "string") {
                console.error("🚨 [Login.js] 예상치 못한 로그인 응답 데이터:", loginUser);
        ...
        .catch(error => {
            ...
            if (error.response && error.response.status === 401) {
                ...
                alert("[Login.js] 로그인 정보가 유효하지 않습니다. 다시 로그인해주세요.");
    ...
    return (
        <article className="login-page-container">
            <section className="login-page-form">
                ...
                <button className="login-page-signup-button" onClick={() => navigate('/memberTerms')}>회원가입</button>
